...
from pipeline.runtime import PROJECT_ROOT, VAULT_PATH
...
THEORY_VAULT_SYSTEM = """You are a knowledge-base curator writing an Obsidian-format theory entry for the listener's vault. The entry will be consumed by a coaching agent that helps the listener apply scientific concepts to a daily practice, and by the listener himself when reviewing the underlying mechanism months later.

Input: a deep-dive brief that synthesises {n} academic/practitioner sources on a single sub-angle.

Output a single markdown file with EXACTLY this structure (Obsidian Flavored Markdown):

```
# {date} — {theme} — {sub_angle} (theory)

**Date:** {date}
**Type:** auto-theory
**Themes:** [[{theme_slug}]]
**Sub-angle:** {sub_angle}
**Source brief:** `briefs/{theme_slug}/{date}--{sub_slug}/deep-dive.md`

## Core Insight
Two to three sentences distilling the most load-bearing mechanism the deep-dive establishes. Stay neuro-biological / physiological — no protocol advice.

## Mechanism
A 200–400-word explanation of HOW the mechanism works at the cellular / systemic level. Cite specific structures (neurons, brain regions, hormones, pathways) by name. Quote concrete numbers from the deep-dive where they exist (effect sizes, half-lives, sample sizes). No motivational framing, no first-person "you should".

## Pathway
A directional chain of the underlying biology, written as one short bullet per step, using `→` arrows where causal flow exists. Example shape:
- inflammation → cytokines released into bloodstream
- cytokines → cross blood-brain barrier → microglia activation
- microglia activation → reduced dopamine release in mesolimbic circuit
- → motivation drop, sickness behavior, training avoidance

## Terminology
A glossary of 4–8 terms central to the deep-dive. Each entry:
**Term** — one-line definition; trigger condition; what it produces; downstream effect.

## See also
Wikilinks that this entry should cross-reference. Always include [[{theme_slug}]]. Add other wikilinks ONLY if the term appears verbatim in the deep-dive's text (e.g. [[amcc-effect]] only if "anterior mid-cingulate cortex" or "aMCC" appears in the deep-dive).

---
*Entry auto-generated by daily-summarizer. Do not edit by hand.*
```

Fill in the template using the deep-dive content. Keep wording dense, specific, mechanistic. No preamble, no closing remarks outside the template.
"""


APPLICATION_VAULT_SYSTEM = """You are a knowledge-base curator writing an Obsidian-format protocol entry for the listener's coaching agent KB.

Input: the application brief from the daily summarizer — which already contains three concrete protocol adjustments and one open question for the listener (Gonzalo).

Output a single markdown file with EXACTLY this structure:

```
# {date} — {theme} — {sub_angle} (application)

**Date:** {date}
**Type:** auto-application
**Themes:** [[{theme_slug}]]
**Sub-angle:** {sub_angle}
**Linked theory:** [[{date}-{theme_slug}-{sub_slug}-theory]]
**Source brief:** `briefs/{theme_slug}/{date}--{sub_slug}/application.md`

## Practical Application
The three concrete adjustments, copied verbatim from the application brief's "Three concrete adjustments to the practice" section. Keep the numbered format and the "— [source mechanism]" attribution.

## When to apply
For each adjustment, one short bullet naming the situation, training block, or affective state that should trigger it. Be concrete: "after any rest day longer than 48h", "when perceived effort > actual HR-zone effort", etc.

## Cues to watch for
Three bullets naming observable signals (behavioural, somatic, or affective) that indicate the listener IS in the situation the protocol targets. The coaching agent uses these as detection criteria.

## Open question
The single sharp question from the application brief's "One question the brief leaves on Gonzalo's desk" section, copied verbatim.

---
*Entry auto-generated by daily-summarizer. Do not edit by hand.*
```

The Practical Application section MUST quote the brief's three adjustments verbatim. The When-to-apply and Cues sections are NEW content you generate based on the adjustments. No preamble, no closing remarks outside the template.
"""
...
def _theory_vault(
    ...
    """Convert deep-dive.md into an Obsidian-format theory entry for the vault.

    Same content as deep-dive (per the no-derivative-sources rule, this is a
    format conversion only — the file is NEVER fed back to NotebookLM as a new
    source). It exists for the Obsidian-side knowledge base, where wikilinks
    and the coaching agent's KB consume it."""
    user = (
        f"Theme: {theme}\nTheme slug: {theme}\n"
        f"Sub-angle: {sub_angle}\nSub-slug: {sub_slug}\n"
        f"Date: {today}\n\n"
...
def _application_vault(
    ...
    """Convert application.md into an Obsidian-format protocol entry for the
    coaching agent KB. Format conversion only — never fed back to NotebookLM."""
    user = (
        f"Theme: {theme}\nTheme slug: {theme}\n"
        f"Sub-angle: {sub_angle}\nSub-slug: {sub_slug}\n"
        f"Date: {today}\n\n"
    ...
    return call_llm(
        model=BRIEF_MODEL,
        system_prompt=APPLICATION_VAULT_SYSTEM,
        user_message=user,
        max_tokens=2000,
    )["text"]


def _write_vault_entries(
    theme: str,
    sub_slug: str,
    today: str,
    theory_md: str,
    application_vault_md: str,
) -> tuple[Path, Path]:
    """Write the two derivative entries into the Obsidian vault at
    `<vault>/gonzalo-book/deep-dive/<theme>/<date>--<sub-slug>/{theory,application}.md`.

    Returns the two paths. Vault git commit is left to the caller so multiple
    briefs in one run can batch into a single submodule commit.
    """
    entry_dir = VAULT_PATH / "gonzalo-book" / "deep-dive" / theme / f"{today}--{sub_slug}"
    entry_dir.mkdir(parents=True, exist_ok=True)
    theory_path = entry_dir / "theory.md"
    application_path = entry_dir / "application.md"
    theory_path.write_text(theory_md)
    application_path.write_text(application_vault_md)
    return theory_path, application_path


def _commit_vault(paths: List[Path], message: str) -> None:
    """Stage + commit the given paths inside the vault submodule. Silent no-op
    if there's nothing to commit (e.g. file content unchanged)."""
    vault_root = VAULT_PATH
    if not (vault_root / ".git").exists():
        # Not a git repo (or submodule pointer missing) — skip silently.
        return
    try:
        ...
        return
    subprocess.run(
        ["git", "-C", str(vault_root), "add", "--", *rel_paths],
        check=False,
        capture_output=True,
    )
    # `git diff --cached --quiet` returns 1 when there are staged changes.
    diff = subprocess.run(
        ["git", "-C", str(vault_root), "diff", "--cached", "--quiet"],
        capture_output=True,
    )
...
def run_cluster(cluster: Dict[str, Any]) -> None:
    ...
    # 5. Vault derivative entries (theory + application) — format conversions
    # of deep-dive.md + application.md for the Obsidian KB / coaching agent.
    # Errors here MUST NOT abort the brief; the audio + visuals already
    # succeeded by this point.
    try:
        print("[poc-v2]   vault theory entry...", flush=True)
        ...
        print("[poc-v2]   vault application entry...", flush=True)
        ...
        theory_path, app_vault_path = _write_vault_entries(
            theme=cluster["theme"],
            sub_slug=sub_slug,
            today=today,
            theory_md=theory_md,
        ...
        print(f"[poc-v2]   wrote vault: {theory_path.relative_to(VAULT_PATH)} + application.md", flush=True)
        _commit_vault(
            paths=[theory_path, app_vault_path],
            message=f"deep-dive: {today} {cluster['theme']} / {cluster['sub_angle']} (theory + application)",
        )
    except Exception as exc:  # noqa: BLE001
        print(f"!! vault entries failed: {type(exc).__name__}: {exc}", flush=True)
